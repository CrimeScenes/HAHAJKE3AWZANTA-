...
local function getPredictedPosition(character)
    ...
    if primaryPart then
        ...
        -- Prediction formula considering speed and velocity
        local predictedPosition = primaryPart.Position + (velocity * timeToPredict)

        -- Adjust for freefall (if player is falling, predict downward movement)
...
local function GetOptimalTargetPoint(character)
    ...
    -- If no favorable parts are found, fallback to the closest hit point logic
    return GetClosestHitPoint(character)  -- Default to closest visible body part
end
...
local function GetClosestPoint(character)
    ...
    return closestPoint  -- Return the closest point based on mouse location
end
